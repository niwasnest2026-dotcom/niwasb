...
export async function POST(request: NextRequest) {
  try {
    console.log('🔄 Creating manual booking for pay_RzMDWPSs34L2tw');
    ...
    if (propertyError || !property) {
      return NextResponse.json({
        success: false,
        error: 'No properties with owner details found'
      }, { status: 404 });
    }
    ...
    // Create or get a user profile for the real payment
    ...
    const bookingData = {
      ...
      notes: `Manual booking created for real payment pay_RzMDWPSs34L2tw - Testing owner details system`,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
    ...
    return NextResponse.json({
      success: true,
      message: 'Manual booking created successfully for pay_RzMDWPSs34L2tw',
      data: {
