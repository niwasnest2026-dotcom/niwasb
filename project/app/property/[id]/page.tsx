        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-8">
            <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6">
              ...
              <div className="bg-gradient-to-br from-rose-50 to-orange-50 border-2 border-rose-200 rounded-2xl p-6 md:ml-8 mt-6 md:mt-0">
                <div className="text-center">
                  ...
                  <div className="text-center">
                    <p className="text-sm text-gray-600 mb-3">Select a room below to book</p>
                    <div className="px-6 py-3 bg-gray-100 text-gray-600 font-bold rounded-xl">
                      Choose Room Type
                    </div>
                  </div>
            ...
            {(property as any).rooms && (property as any).rooms.length > 0 && (
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {(property as any).rooms.map((room: any) => (
                    <div
                      key={room.id}
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                          {room.floor_number && (
                            <div className="flex items-center text-gray-600">
                              ...
                              <span className="ml-1">{room.floor_number}</span>
                            </div>
                          )}
                          {room.room_size_sqft && (
                            <div className="flex items-center text-gray-600">
                              <span className="font-medium">Size:</span>
                              <span className="ml-1">{room.room_size_sqft} sqft</span>
                            </div>
                          )}
                          <div className="flex items-center text-gray-600">
                            <span className="font-medium">Total Beds:</span>
                            <span className="ml-1">{room.total_beds}</span>
                          </div>
                          <div className="flex items-center text-gray-600">
                            <span className="font-medium">Available:</span>
                            <span className="ml-1">{room.available_beds}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t">
                          <div>
                            <div className="text-2xl font-bold text-gray-900">
                              ₹{room.price_per_person.toLocaleString()}
                            </div>
                            <div className="text-sm text-gray-600">per person/month</div>
                          </div>
                          {room.available_beds > 0 ? (
                            <Link
                              href={`/payment?propertyId=${property.id}&roomId=${room.id}`}
                              className="px-6 py-2 bg-primary hover:bg-primary-dark text-white font-semibold rounded-lg transition-all"
                            >
                              Book Now
                            </Link>
                          ) : (
                            <span className="px-6 py-2 bg-gray-300 text-gray-600 font-semibold rounded-lg cursor-not-allowed">
                              Full
                            </span>
                          )}
                        </div>
